...
int main(void)
{
    ...
    GLfloat t_vertex_data_dim3[NB_TEAMS][NB_DAYS * 2 + 1][(nbDivCylindre * 2 + 2) * 3];  // * 3 car {x,y,z} pour chaque point

    for(int team = 0; team < NB_TEAMS; team++)
    {
        for(int i = 0; i < NB_DAYS * 2 + 1; i++)
        {
            // A:0
            t_vertex_data_dim3[team][i][0]     = t_vertex_data[team][0];
            t_vertex_data_dim3[team][i][0 + 1] = t_vertex_data[team][0 + 1];
            t_vertex_data_dim3[team][i][0 + 2] = t_vertex_data[team][0 + 2];

            // B:0
            t_vertex_data_dim3[team][i + 1][3]     = t_vertex_data[team][2 * 3];
            t_vertex_data_dim3[team][i + 1][3 + 1] = t_vertex_data[team][2 * 3 + 1];
            t_vertex_data_dim3[team][i + 1][3 + 2] = t_vertex_data[team][2 * 3 + 2];


            for(int points = 6; points < (nbDivCylindre * 2 + 2) * 3; points+=3)
            {

                t_vertex_data_dim3[team][i][points]     = t_vertex_data_dim3[team][points - 6][i]     ;
                t_vertex_data_dim3[team][i][points + 1] = t_vertex_data_dim3[team][points - 6][i + 1];
                t_vertex_data_dim3[team][i][points + 2] = t_vertex_data_dim3[team][points - 6][i + 2];

                t_vertex_data_dim3[team][i + 1][points]     = t_vertex_data_dim3[team][points - 3][i]     ;
                t_vertex_data_dim3[team][i + 1][points + 1] = t_vertex_data_dim3[team][points - 3][i + 1];
                t_vertex_data_dim3[team][i + 1][points + 2] = t_vertex_data_dim3[team][points - 3][i + 2];
...
*/
    ...
    GLuint cylindre_vertexbuffer[NB_TEAMS][NB_DAYS*2+1];
    for(int team = 0; team < NB_TEAMS; team++)
    {
        for(int partieDeCourbe = 0; partieDeCourbe < NB_DAYS * 2 + 1; partieDeCourbe++)
        {
            glGenBuffers(1, &cylindre_vertexbuffer[team][partieDeCourbe]);
            glBindBuffer(GL_ARRAY_BUFFER, cylindre_vertexbuffer[team][partieDeCourbe]);
            glBufferData(
                    GL_ARRAY_BUFFER,
                    sizeof(t_vertex_data_dim3[team][partieDeCourbe * (nbDivCylindre*2+2)]),
                    t_vertex_data_dim3[team][partieDeCourbe * (nbDivCylindre*2+2)],
                    GL_STATIC_DRAW
                    );
        }
    }
