...
enum ImGuiButtonFlags_
{
    ...
    ImGuiButtonFlags_PressedOnDragDropHold  = 1 << 12,  // press when held into while we are drag and dropping another item (used by e.g. tree nodes, collapsing includes)
...
// Transient per-window flags, clear at the beginning of the frame. For child window, inherited from parent on first Begin().
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    float                   CursorAnim;             // timer for cursor blink, clear on every user action so the cursor reappears immediately
...
struct ImGuiSettingsHandler
{
    ...
    void*       (*ReadOpenFn)(ImGuiContext* ctx, ImGuiSettingsHandler* handler, const char* name);              // Read: Called when entering into a new ini entry e.g. "[Render][Name]"
...
struct ImGuiContext
{
    ...
    ImVector<ImGuiPopupData>BeginPopupStack;                    // Which level of BeginPopup() we are in (clear every frame)
    ...
    bool                    NavMoveFromClampedRefRect;          // Set by manual scrolling, if we scroll to a point where NavId isn't visible we clear navigation from visible items
    ...
    // Render
    ImDrawData              DrawData;                           // Main ImDrawData instance to pass screen information to the user
    ImDrawDataBuilder       DrawDataBuilder;
    float                   DimBgRatio;                         // 0.0..1.0 animation when fading in a dimming background (for modal window and CTRL+TAB list)
    ImDrawList              BackgroundDrawList;                 // First draw list to be rendered.
    ImDrawList              ForegroundDrawList;                 // Last draw list to be rendered. This is where we the screen software mouse cursor (if io.MouseDrawCursor is set) and most debug overlays.
...
// Transient per-window data, clear at the beginning of the frame. This used to be called ImGuiDrawContext, hence the DC variable name in ImGuiWindow.
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImVec2                  WindowPadding;                      // Render padding at the time of Begin().
    float                   WindowRounding;                     // Render rounding at the time of Begin().
    float                   WindowBorderSize;                   // Render border size at the time of Begin().
    ...
    ImGuiWindowTempData     DC;                                 // Temporary per-window data, clear at the beginning of the frame. This used to be called ImGuiDrawContext, hence the "DC" variable name.

    // The best way to understand what those rectangles are is to use the 'Metrics -> Tools -> Show windows rectangles' viewer.
    // The main 'OuterRect', omitted as a field, is window->Rect().
    ImRect                  OuterRectClipped;                   // == Render->Rect() just after setup in Begin(). == window->Rect() for root window.
...
struct ImGuiTabItem
{
    ...
    int                 NameOffset;             // When Render==NULL, offset to name within parent ImGuiTabBar::TabsNames
