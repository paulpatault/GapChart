...
/*
 ...
 - C++: this is a very C-ish codebase: we don't rely on C++11, we don't include any C++ includes, and ImGui:: is a namespace.
 ...
 - Dear ImGui never touches or knows about your GPU state. The only function that knows about GPU is the draw function that you provide.
   Effectively it means you can create widgets at any time in your code, regardless of considerations of being in "update" vs "screen"
 ...
 HOW A SIMPLE RENDERING FUNCTION MAY LOOK LIKE:
    ...
    {
       // TODO: Setup screen state: alpha-blending enabled, no face culling, no depth testing, scissor enabled
 ...
 - 2018/09/06 (1.65) - renamed stb_truetype.h to imstb_truetype.h, stb_textedit.h to imstb_textedit.h, and stb_rect_pack.h to imstb_rectpack.h.
                       If you were conveniently using the imgui copy of those STB includes in your project you will have to update your include paths.
 ...
 - 2018/02/16 (1.60) - obsoleted the io.RenderDrawListsFn callback, you can call your graphics engine screen function after ImGui::Render(). Use ImGui::GetDrawData() to retrieve the ImDrawData* to Display.
 ...
 - 2015/12/27 (1.48) - fixed ImDrawList::AddRect() which used to screen a rectangle 1 px too large on each axis.
 ...
 - 2015/07/14 (1.43) - add new ImFontAtlas::AddFont() API. For the old AddFont***, moved the 'font_no' parameter of ImFontAtlas::AddFont** functions to the ImFontConfig structure.
                       you need to screen your textured triangles with bilinear filtering to benefit from sub-pixel positioning of text.
 - 2015/07/08 (1.43) - switched rendering data to use indexed rendering. this is saving a fair amount of CPU/GPU and enables us to get anti-aliasing for a marginal cost.
                     ...
                     - each ImDrawList now contains both a vertex buffer and an index buffer. For each command, screen ElemCount/3 triangles using indices from the index buffer.
                     - if you REALLY cannot screen indexed primitives, you can call the draw_data->DeIndexAllBuffers() method to de-index the buffers. This is slow and a waste of CPU/GPU. Prefer using indexed rendering!
 ...
 - 2015/01/11 (1.30) - big font/image API change! now loads TTF file. allow for multiple fonts. no need for a PNG loader.
              ...
              (1.30) - added texture identifier in ImDrawCmd passed to your screen function (we can now screen images). make sure to set io.Fonts->TexID.
   ...
   - Solving ID conflict in a more general manner:
     ...
     Begin("Render");
       for (int i = 0; i < 100; i++)
       {
         ...
         Button("Click");     // Label = "Click",  ID = hash of ("Render", i, "Click")
         PopID();
       }
       for (int i = 0; i < 100; i++)
       {
         ...
         PushID(obj);
         Button("Click");     // Label = "Click",  ID = hash of ("Render", obj pointer, "Click")
         PopID();
       }
       for (int i = 0; i < 100; i++)
       {
         MyObject* obj = Objects[i];
         PushID(obj->Name);
         Button("Click");     // Label = "Click",  ID = hash of ("Render", obj->Name, "Click")
         PopID();
       }
       End();
...
// Render resizing from edges (when io.ConfigWindowsResizeFromEdges = true and ImGuiBackendFlags_HasMouseCursors is set in io.BackendFlags by back-end)
...
ImGuiStyle::ImGuiStyle()
{
    ...
    DisplayWindowPadding    = ImVec2(19,19);    // Render position are clamped to be visible within the Display area by at least this amount. Only applies to regular windows.
...
// Zero-terminated string hash, with support for ### to clear back to seed value
// We support a syntax of "label###id" where only "###id" is included in the hash, and only "label" gets displayed.
// Because this syntax is rarely used we are optimizing for the common case.
// - If we reach ### in the string we discard the hash so far and clear to the seed.
...
// Internal ImGui functions to screen text
...
void ImGui::UpdateMouseMovingWindowEndFrame()
{
    ...
    {
        // Find the top-most window between HoveredWindow and the top-most Modal Render.
...
void ImGui::NewFrame()
{
    ...
    // Create implicit/fallback window - which we will only screen it if the user has added something to it.
...
void ImGui::Initialize(ImGuiContext* context)
{
    ...
    // Add .ini handle for ImGuiWindow type
    {
        ...
        ini_handler.TypeName = "Render";
        ini_handler.TypeHash = ImHashStr("Render");
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list)
{
    ...
    //       Your own engine or screen API may use different parameters or function calls to specify index sizes.
...
// When using this function it is sane to ensure that float are perfectly rounded to integer values, to that e.g. (int)(max.x-min.x) in user's screen produce correct result.
...
void ImGui::Render()
{
    ...
    // Gather ImDrawList to screen (for each active window)
    ...
    // (Legacy) Call the Render callback function. The current prefer way is to let the user retrieve GetDrawData() and call the screen function themselves.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
void ImGui::ResetMouseDragDelta(int button)
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(button >= 0 && button < IM_ARRAYSIZE(g.IO.MouseDown));
    // NB: We don't need to clear g.IO.MouseDragMaxDistanceSqr
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    IM_ASSERT(name != NULL && name[0] != '\0');     // Render name required
    IM_ASSERT(g.FrameScopeActive);                  // Forgot to call ImGui::NewFrame()
    ...
    if (first_begin_of_the_frame)
    {
        ...
        if (g.NavWindowingList != NULL && (window->Flags & ImGuiWindowFlags_NoNavFocus) == 0)   // Render titles visible when using CTRL+TAB
        ...
        // We clear Size/ContentSize for reappearing popups/tooltips early in this function, so further code won't be tempted to use the old size.
        ...
        // Force round operator last to ensure that e.g. (int)(max.x-min.x) in user's screen code produce correct result.
        ...
        // Since 1.71, child window can screen their decoration (bg color, border, scrollbars, etc.) within their parent to save a draw call.
    ...
    // Don't screen if style alpha is 0.0 at the time of Begin(). This is arbitrary and inconsistent but has been there for a long while (may remove at some point)
    if (style.Alpha <= 0.0f)
        window->HiddenFramesCanSkipItems = 1;
...
void ImGui::BeginTooltipEx(ImGuiWindowFlags extra_flags, bool override_previous_tooltip)
{
    ...
    if (override_previous_tooltip)
        if (ImGuiWindow* window = FindWindowByName(window_name))
            if (window->Active)
            {
                // Hide previous tooltip from being displayed. We can't easily "clear" the content of a window so we create a new one.
                window->Hidden = true;
                window->HiddenFramesCanSkipItems = 1;
                ImFormatString(window_name, IM_ARRAYSIZE(window_name), "##Tooltip_%02d", ++g.TooltipOverrideCount);
            }
...
// One open popup per level of the popup hierarchy (NB: when assigning we clear the Render member of ImGuiPopupRef to NULL)
void ImGui::OpenPopupEx(ImGuiID id)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* parent_window = g.CurrentWindow;
    int current_stack_size = g.BeginPopupStack.Size;
    ImGuiPopupData popup_ref; // Tagged as new ref as Render will be set back to NULL if we write this into OpenPopupStack.
    ...
    if (quadrant == g.NavMoveDir)
    {
        ...
        if (dist_box == result->DistBox)
        {
            ...
            else if (dist_center == result->DistCenter)
            {
                ...
                // this is fairly easy. This rule ensures that all buttons with DELTA_X==dy==0 will end up being linked in order of appearance along the x axis.
...
static void NavUpdateWindowingHighlightWindow(int focus_change_dir)
{
    ...
    if (window_target) // Don't clear windowing target if there's a single window in the list
...
// Render has already passed the IsWindowNavFocusable()
...
// When this returns true you need to: a) call SetDragDropPayload() exactly once, b) you may screen the payload visual/description, c) call EndDragDropSource()
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
        ...
        static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list, const char* label)
        {
            ...
            if (window && !window->WasActive)
                ImGui::Text("(Note: owning Render is inactive: DrawList is not being rendered!)");
        ...
        static void NodeWindows(ImVector<ImGuiWindow*>& windows, const char* label)
        {
            ...
            for (int i = 0; i < windows.Size; i++)
                Funcs::NodeWindow(windows[i], "Render");
            ImGui::TreePop();
        }
    ...
    if (ImGui::TreeNode("Popups", "Popups (%d)", g.OpenPopupStack.Size))
    {
        for (int i = 0; i < g.OpenPopupStack.Size; i++)
        {
            ...
            ImGui::BulletText("PopupID: %08x, Render: '%s'%s%s", g.OpenPopupStack[i].PopupId, window ? window->Name : "NULL", window && (window->Flags & ImGuiWindowFlags_ChildWindow) ? " ChildWindow" : "", window && (window->Flags & ImGuiWindowFlags_ChildMenu) ? " ChildMenu" : "");
        }
        ImGui::TreePop();
    }
